...
data.addChannelOption((option) =>
	option.setName('channel').setDescription('The channel to manage').setRequired(true).addChannelTypes(ChannelType.GuildText)
);
...
export default newSlashCommand({
	...
	execute: async (i) => {
		...
		if (whitelist && checkChannel) {
			if (!broadcast || checkChannel.canBroadcast == broadcast)
				return i.reply({ content: 'This channel is already whitelisted', ephemeral: true });
